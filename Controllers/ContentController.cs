...
namespace StellarStreamAPI.Controllers
{
    ...
    public class ContentController : ControllerBase
    {
        ...
        public async Task<IActionResult> NasaLibrary(string? title, string? center, string? nasaId, string? mediaType, [FromQuery]string[]? keywords, DateTime? startDate, DateTime? endDate, string? description, string? contentType, int count = 10, int offset = 0)
        {
            var result = await _dbContext.GetNasaImages(count, offset, title, center, nasaId, mediaType, keywords, startDate, endDate, description, contentType);
